...
namespace MovieFinder2025.ViewModels
{
    public partial class MoviesViewModel : ObservableObject
    {
        ...
        public MoviesViewModel(IDialogCoordinator coordinator)
        {
            ...
            // 시계작업
            CurrDateTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"); // 처음 1초가 지나기 전 시간 출력
        ...
        public async Task SearchMovie()
        {
            ...
            var controller = await dialogCoordinator.ShowProgressAsync(this, "대기 중", "검색 중...");
        ...
        {
            string tmdb_apikey = "38ed78471444c0efea607fd1e87ec1be"; // TMDB에서 신청한 API 키
        ...
        public async Task AddFavoriteMovies()
        {
            ...
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Common.CONNSTR))
                {
                    ...
                    if (resultCnt > 0)
                    {
                        Common.LOGGER.Info($"{SelectedMovieItem.Title} 즐겨찾기 추가 완료");
                        await this.dialogCoordinator.ShowMessageAsync(this, "즐겨찾기 추가 완료", "즐겨찾기에 추가했습니다.");
                    }
                    else
                    {
                        await this.dialogCoordinator.ShowMessageAsync(this, "추가 오류", "영화를 추가하는 도중 오류가 발생했습니다.");
                    }
                }
            }
            catch(MySqlException ex)
            {
                ...
                {
                    Common.LOGGER.Warn($"{SelectedMovieItem.Title} 즐겨찾기 추가 실패");
                    await this.dialogCoordinator.ShowMessageAsync(this, "추가 실패", "이미 추가된 영화입니다.");
                }
                else
                {
                    await this.dialogCoordinator.ShowMessageAsync(this, "추가 오류", ex.Message);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task ViewFavoriteMovies()
        {
            ...
            {
                await this.dialogCoordinator.ShowMessageAsync(this, "오류", "즐겨찾기 보기 실패");
                Common.LOGGER.Fatal(ex.Message);
            }
        }
        ...
        public async Task DelFavoriteMovies()
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Common.CONNSTR))
                {
                    ...
                    if (resultCnt > 0)
                    {
                        Common.LOGGER.Info($"{SelectedMovieItem.Title} 즐겨찾기 삭제 완료");
                        await this.dialogCoordinator.ShowMessageAsync(this, "즐겨찾기 삭제 완료", "즐겨찾기에서 삭제했습니다.");
                        PosterUri = new Uri("/No_Picture.png", UriKind.RelativeOrAbsolute);
                        await ViewFavoriteMovies();
                    }
                    else
                    {
                        Common.LOGGER.Warn($"{SelectedMovieItem.Title} 즐겨찾기 삭제 실패");
                        await this.dialogCoordinator.ShowMessageAsync(this, "삭제 실패", "삭제할 영화가 없습니다.");
                    }
                }
            }
            catch (Exception ex)
            {
                await this.dialogCoordinator.ShowMessageAsync(this, "삭제 오류", "영화를 삭제하는 도중 오류가 발생했습니다.");
                Common.LOGGER.Fatal(ex.Message);
            }
        }
